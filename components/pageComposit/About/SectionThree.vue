<template>
    <section class="grid grid-cols-1 md:grid-cols-2 gap-6 items-center px-6 py-20 relative full-container overflow-x-clip">
      <div class="absolute w-[40rem] h-[40rem] opacity-50 top-1/2 left-full -translate-x-1/2 -translate-y-1/2 bg-radial-gradient blur-3xl"></div>
      <img src="/batik-element.svg" alt="" class="absolute top-[-25%] left-[-10%] scale-x-[-1] scale-y-[-1] z-[-1]">
      <img class="rounded-t-full w-2/3" src="/images/patung-batik.png" alt="patung batik">
      <div class="text-center md:text-left">
        <h3 class="text-3xl font-bold">Museum Batik Indonesia</h3>
        <p class="mt-4">
          Museum Batik Indonesia didirikan sebagai wujud penetapan Batik Indonesia atas masuknya batik ke dalam Representative List of the
          Intangible Cultural Heritage of Humanity oleh UNESCO pada tanggal 2 Oktober 2009.
        </p>
      </div>
    </section>
  </template>
  